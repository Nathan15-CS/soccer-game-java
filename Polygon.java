...
//    for (Point p : shape) {
			...
			double x = ((p.x - center.x) * Math.cos(Math.toRadians(rotation)))
					- ((p.y - center.y) * Math.sin(Math.toRadians(rotation))) + center.x + position.x;
			double y = ((p.x - center.x) * Math.sin(Math.toRadians(rotation)))
					+ ((p.y - center.y) * Math.cos(Math.toRadians(rotation))) + center.y + position.y;
	...
	/**
	 * Checks if this polygon collides with another polygon. It checks if any point
	 * of the other polygon is inside this polygon.
	 *
	...
	public boolean collides(Polygon other) {
		...
		for (int i = 0; i < otherPoints.length; i++) {
			if (this.contains(otherPoints[i]) == true) {
				return true;
			}
		}
		return false;
